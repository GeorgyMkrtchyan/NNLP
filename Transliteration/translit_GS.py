    ...
    def __init__(self, hidden_size, max_len=512):
        ...
        for pos in range(max_len):
            for i in range(hidden_size//2):
                denominator=10000 ** ((2*i)/hidden_size)
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask):
        ...
        q_k = torch.matmul(query, key.transpose(-2, -1)) / np.sqrt(query.size(-1))
        if mask is not None:
            q_k = q_k.masked_fill(mask==0, -1e10)
        attn_weights = F.softmax(q_k, dim=-1)
        if self.dropout is not None:
...
def train(source_strings, target_strings,writer,d_e,d_a,d_res,d_rel,lr_p):
    ...
    for epoch in range(1,train_config['n_epochs']+1):
        ...
        model.train()
        result=run_epoch(train_dataloader, model,
...
def main(d_e,d_a,d_res,d_rel,lr_p):
    #print("type1"+str(type(d_e)))
    #print("type2"+str(type(d_a)))
    #print("type3"+str(type(d_res)))
    #print("type4"+str(type(d_rel)))
    #print("type5"+str(type(lr_p)))


    writer = SummaryWriter(f"train_search/d_e={d_e}_d_a={d_a}_d_res={d_res}_d_rel={d_rel}_lr_p={lr_p}")
    seed_val = 42
    ...
    #learnable_params = train(train_source_strings, train_target_strings,writer)
    learnable_params = train(train_source_strings, train_target_strings,writer,d_e,d_a,d_res,d_rel,lr_p)
...
if __name__=='__main__':
  import sys
  #print(sys.argv[0])
  main(float(sys.argv[2]),float(sys.argv[3]),float(sys.argv[4]),float(sys.argv[5]),float(sys.argv[6]))
